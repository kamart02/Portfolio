...
namespace diagnostic {
    ...
    string arrayToString(uint64_t const *seq, size_t size) {
        string arrayString = "";
        arrayString.append("\"");
        if (size != 0) {
            for (size_t i = 0; i < size - 1; i++) {
                arrayString.append(to_string(seq[i]));
                arrayString.append(" ");
            }
            arrayString.append(to_string(seq[size - 1]));
        }
        arrayString.append("\"");
